...
const plans = [
  ...
  },
  {
    ...
    note: "Typical range: $1M – $50M",
    highlight: true,
  },
  {
    name: "Industrial",
    tagline: "Warehouses & Plants",
    range: "$60–$180",
    unit: "per sq ft",
    color: "#5b9cf6",
    features: [
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 64, opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(40px)", transition: "all 0.7s ease" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 16 }}>
            ...
            <span style={{ fontFamily: "Barlow Condensed", fontWeight: 600, fontSize: 13, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--amber)" }}>Transparent Pricing</span>
            <div style={{ width: 40, height: 2, background: "var(--amber)" }} />
          </div>
          <h2 style={{ fontSize: "clamp(48px, 6vw, 80px)", color: "var(--white)", marginBottom: 16 }}>PRICING<br /><span style={{ color: "var(--amber)" }}>OVERVIEW</span></h2>
          <p style={{ fontSize: 16, color: "var(--concrete-light)", maxWidth: 520, margin: "0 auto", lineHeight: 1.7 }}>
            We believe in transparent, competitive pricing. Below are typical cost ranges — use our AI Estimator for a personalized quote.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 2, alignItems: "start" }}>
          {plans.map((p, i) => (
            <div key={i} style={{
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = p.color; (e.currentTarget as HTMLElement).style.color = "var(--steel)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = p.highlight ? p.color : "transparent"; (e.currentTarget as HTMLElement).style.color = p.highlight ? "var(--steel)" : p.color; }}
              >Get Detailed Estimate →</a>
            </div>
          ))}
        </div>

        <p style={{ textAlign: "center", marginTop: 40, fontSize: 13, color: "var(--concrete)", fontStyle: "italic" }}>
          * All prices are estimated ranges. Final quotes depend on scope, location, materials, and market conditions.
        </p>
      </div>
    </section>
  );
